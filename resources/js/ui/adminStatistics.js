document.addEventListener('DOMContentLoaded', () => { 
    ...
    // --- Gestion affichage / activation des dates personnalisées ---

    function updateCustomRangeState() {
        if (!customRange || !startInput || !endInput || !applyBtn || !periodSelect) {
            return;
        }
    ...
    if (periodSelect) {
        periodSelect.addEventListener('change', () => {
            // Met à jour l'affichage / activation des dates
    ...
    if (applyBtn) {
        applyBtn.addEventListener('click', () => {
            if (applyBtn.disabled) return; // sécurité
